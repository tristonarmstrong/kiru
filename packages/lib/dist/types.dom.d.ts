...
export type { HTMLTagToElement, SVGTagToElement, HtmlElementAttributes, HtmlElementBindableProps, SvgElementAttributes, SvgGlobalAttributes, GlobalAttributes, StyleObject, };
type HTMLTagToElement<T extends keyof HtmlElementAttributes> = T extends keyof HTMLElementTagNameMap ? HTMLElementTagNameMap[T] : T extends keyof HTMLElementDeprecatedTagNameMap ? HTMLElementDeprecatedTagNameMap[T] : never;
type SVGTagToElement<T extends keyof SvgElementAttributes> = T extends keyof SVGElementTagNameMap ? SVGElementTagNameMap[T] : never;
type NumericStyleKeys = "bottom" | "gap" | "inset" | "insetBlock" | "insetBlockEnd" | "insetBlockStart" | "insetInline" | "insetInlineEnd" | "insetInlineStart" | "left" | "margin" | "marginBlock" | "marginBlockEnd" | "marginBlockStart" | "marginInline" | "marginInlineEnd" | "marginInlineStart" | "padding" | "paddingBlock" | "paddingBlockEnd" | "paddingBlockStart" | "paddingInline" | "paddingInlineEnd" | "paddingInlineStart" | "right" | "top" | "height" | "maxHeight" | "maxWidth" | "minHeight" | "minWidth" | "width" | "flex" | "flexBasis" | "flexGrow" | "flexShrink" | "order" | "columnGap" | "gridAutoColumns" | "gridAutoRows" | "gridColumnGap" | "gridRowGap" | "rowGap" | "borderBottomWidth" | "borderImageOutset" | "borderImageSlice" | "borderImageWidth" | "borderLeftWidth" | "borderRadius" | "borderRightWidth" | "borderSpacing" | "borderTopWidth" | "borderWidth" | "fontSize" | "letterSpacing" | "lineHeight" | "tabSize" | "textIndent" | "wordSpacing" | "scrollMargin" | "scrollMarginBlock" | "scrollMarginBlockEnd" | "scrollMarginBlockStart" | "scrollMarginBottom" | "scrollMarginInline" | "scrollMarginInlineEnd" | "scrollMarginInlineStart" | "scrollMarginLeft" | "scrollMarginRight" | "scrollMarginTop" | "scrollPadding" | "scrollPaddingBlock" | "scrollPaddingBlockEnd" | "scrollPaddingBlockStart" | "scrollPaddingBottom" | "scrollPaddingInline" | "scrollPaddingInlineEnd" | "scrollPaddingInlineStart" | "scrollPaddingLeft" | "scrollPaddingRight" | "scrollPaddingTop" | "animationDelay" | "animationDuration" | "transitionDelay" | "transitionDuration" | "rotate" | "scale" | "translate" | "boxShadow" | "zIndex" | "opacity";
type FunctionKeys<T> = {
    [K in keyof T]: T[K] extends (...args: any) => any ? K : never;
}[keyof T];
type AllStyleRules = Omit<CSSStyleDeclaration, typeof Symbol.iterator | FunctionKeys<CSSStyleDeclaration> | "length" | "parentRule"> & {
    [Key in `--${string}`]: string | number;
};
type StyleObject = Prettify<Partial<{
    [Key in keyof AllStyleRules & string]: Key extends NumericStyleKeys ? Signalable<number | string | undefined> : Signalable<AllStyleRules[Key] | undefined>;
}>>;
...
type InputMode = "decimal" | "email" | "none" | "numeric" | "search" | "text" | "tel" | "url";
type LanguageCode = "en" | "fr" | "es" | "de" | "it" | "pt" | "ru" | "zh" | string;
type MediaQuery = string;
type MediaType = string;
type MimeType = string;
type ReferrerPolicy = "no-referrer" | "no-referrer-when-downgrade" | "origin" | "origin-when-cross-origin" | "unsafe-url";
type AnchorRel = "alternate" | "author" | "bookmark" | "external" | "help" | "license" | "next" | "nofollow" | "noreferrer" | "noopener" | "prev" | "search" | "tag";
type LinkRel = "alternate" | "archives" | "author" | "bookmark" | "canonical" | "external" | "first" | "help" | "icon" | "last" | "license" | "next" | "nofollow" | "noreferrer" | "pingback" | "preconnect" | "prefetch" | "preload" | "prev" | "search" | "sidebar" | "stylesheet" | "tag" | "up";
...
type NoChildEventsElement = HTMLAreaElement | HTMLBaseElement | HTMLBRElement | HTMLEmbedElement | HTMLHRElement | HTMLImageElement | HTMLInputElement | HTMLLinkElement | HTMLMetaElement | HTMLSelectElement | HTMLSourceElement | HTMLTrackElement | HTMLTextAreaElement;
type DomElement = Element;
declare global {
    namespace Kiru {
        type DOMEvent<E = Event, C = unknown, T = unknown> = Omit<E, "target" | "currentTarget"> & {
        ...
        interface BaseEvent<T extends DomElement = DomElement> extends DOMEvent<Event, T> {
        }
        interface AnimationEvent<T extends DomElement = DomElement> extends DOMEvent<NativeAnimationEvent, T> {
        }
        interface ClipboardEvent<T extends DomElement = DomElement> extends DOMEvent<NativeClipboardEvent, T> {
        }
        interface CompositionEvent<T extends DomElement = DomElement> extends DOMEvent<NativeCompositionEvent, T> {
        }
        interface DragEvent<T extends DomElement = DomElement> extends DOMEvent<NativeDragEvent, T> {
        }
        interface FocusEvent<T extends DomElement = DomElement> extends DOMEvent<NativeFocusEvent, T> {
        }
        interface FormEvent<T extends DomElement = DomElement> extends DOMEvent<Event, T> {
        }
        interface KeyboardEvent<T extends DomElement = DomElement> extends DOMEvent<NativeKeyboardEvent, T> {
        }
        interface MouseEvent<T extends DomElement = DomElement> extends DOMEvent<NativeMouseEvent, T> {
        }
        interface PointerEvent<T extends DomElement = DomElement> extends DOMEvent<NativePointerEvent, T> {
        }
        interface SubmitEvent<T extends DomElement = DomElement> extends DOMEvent<NativeSubmitEvent, T> {
        }
        interface TouchEvent<T extends DomElement = DomElement> extends DOMEvent<NativeTouchEvent, T> {
        }
        interface ToggleEvent<T extends DomElement = DomElement> extends DOMEvent<NativeToggleEvent, T> {
        }
        interface TransitionEvent<T extends DomElement = DomElement> extends DOMEvent<NativeTransitionEvent, T> {
        }
        interface UIEvent<T extends DomElement = DomElement> extends DOMEvent<NativeUIEvent, T> {
        }
        interface WheelEvent<T extends DomElement = DomElement> extends DOMEvent<NativeWheelEvent, T> {
        }
        type BaseEventHandler<T extends DomElement = DomElement> = EventHandler<BaseEvent<T>>;
        type ClipboardEventHandler<T extends DomElement = DomElement> = EventHandler<ClipboardEvent<T>>;
        type CompositionEventHandler<T extends DomElement = DomElement> = EventHandler<CompositionEvent<T>>;
        type DragEventHandler<T extends DomElement = DomElement> = EventHandler<DragEvent<T>>;
        type FocusEventHandler<T extends DomElement = DomElement> = EventHandler<FocusEvent<T>>;
        type FormEventHandler<T extends DomElement = DomElement> = EventHandler<FormEvent<T>>;
        type KeyboardEventHandler<T extends DomElement = DomElement> = EventHandler<KeyboardEvent<T>>;
        type MouseEventHandler<T extends DomElement = DomElement> = EventHandler<MouseEvent<T>>;
        type TouchEventHandler<T extends DomElement = DomElement> = EventHandler<TouchEvent<T>>;
        type PointerEventHandler<T extends DomElement = DomElement> = EventHandler<PointerEvent<T>>;
        type UIEventHandler<T extends DomElement = DomElement> = EventHandler<UIEvent<T>>;
        type WheelEventHandler<T extends DomElement = DomElement> = EventHandler<WheelEvent<T>>;
        type AnimationEventHandler<T extends DomElement = DomElement> = EventHandler<AnimationEvent<T>>;
        type ToggleEventHandler<T extends DomElement = DomElement> = EventHandler<ToggleEvent<T>>;
        type TransitionEventHandler<T extends DomElement = DomElement> = EventHandler<TransitionEvent<T>>;
        type CustomEventAttributes = {
            [Key in keyof Kiru.CustomEvents as `on:${Key}`]?: (event: CustomEvent<Kiru.CustomEvents[Key]>) => void;
        };
        interface EventAttributes<T extends DomElement = DomElement> extends CustomEventAttributes {
...
type MediaElementBindableProps = BindableProp<"volume", number> & BindableProp<"playbackRate", number> & BindableProp<"currentTime", number>;
interface HtmlElementBindableProps {
    input: BindableProp<"value", string | number> & BindableProp<"checked", boolean>;
...
interface HtmlElementAttributes {
    ...
    video: HTMLMediaElementAttrs & {
        ...
        width?: string | number;
        height?: string | number;
        onenterpictureinpicture?: (this: HTMLVideoElement, ev: PictureInPictureEvent) => void;
        onleavepictureinpicture?: (this: HTMLVideoElement, ev: PictureInPictureEvent) => void;
    };
}
...
interface SvgElementAttributes {
    ...
    filter: SvgColorInterpolationFilters & SvgColorInterpolation & {
        x?: string | number;
        y?: string | number;
        width?: string | number;
        height?: string | number;
...
//# sourceMappingURL=types.dom.d.ts.map